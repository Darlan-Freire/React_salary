...
function calculateSalaryFrom(fullSalary) {
  ...
  const percentINSS = Number((discountINSS / fullSalary) * 100).toFixed(2);
  const percentIRPF = Number((discountIRPF / fullSalary) * 100).toFixed(2);

  const percentNetSalary = Number((netSalary / fullSalary) * 100).toFixed(2);
